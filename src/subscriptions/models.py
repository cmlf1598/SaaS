...
class Subscription(models.Model):
    ...
    features = models.TextField(help_text="Features for pricing, separated by new line",
                                blank=True, null=True)

    def __str__(self):
        return f"{self.name}" # to show the actual name in the admin menu
...
class SubscriptionPrice(models.Model):
    ...
    def save(self, *args, **kwargs):
        if (not self.stripe_id and 
            ...
            self.stripe_id = stripe_id
        super().save(*args, **kwargs)
        #make sure only 1 price type is featured
...
class UserSubscription(models.Model):
    ...
    on_delete=models.SET_NULL, null=True, blank=True) # user can or cannot have a subscription
    stripe_id = models.CharField(max_length=120, null=True, blank=True)
...
def user_sub_post_save(sender, instance, *args, **kwargs):
    ...
    if not ALLOW_CUSTOM_GROUPS:
        ...
        if subscription_obj is not None: 
            subs_qs = subs_qs.exclude(id=subscription_obj.id) # active Subscriptions
        subs_groups = subs_qs.values_list("groups__id", flat=True)
        subs_groups_set = set(subs_groups)
        # groups_ids = groups.values_list('id', flat=True) # ids as a list 
        current_groups = user.groups.all().values_list('id', flat=True)
        groups_ids_set = set(groups_ids) # if group is a set, then the union of them 
        current_groups_set = set(current_groups) - subs_groups_set # my current groups 
